...
const CWGameUI = (() => {
  ...
  function _showCombatSelect() {
    ...
    if (_defileInProgress) { _showToast('Impossible de quitter le Défilé en cours !', 'error'); return; }
    ...
    const modes = [
      ...
      { id:'fashion-week', icon:'🗓️', name:'Semaine de Mode',
        desc: `${(state.player.fashionWeekCurrency || 0).toLocaleString('fr-FR')} 🌠 — gère l'agenda de tes actrices`,
        unlocked: true, lockedDesc: '' },
    ];
    ...
    grid?.querySelectorAll('.cs-card:not(.locked)').forEach(card => {
      card.addEventListener('click', () => {
        ...
        if (mode === 'defile-random') {
          const state = CWGameState.get();
          const usedLines = new Set();
          const picked = [];
          const shuffled = [...state.player.collection].sort(() => Math.random() - 0.5);
          for (const inst of shuffled) {
            if (picked.length >= 9) break;
            const def = CWGameState.getCharDef(inst.charId);
            if (!def || usedLines.has(def.evolutionLine)) continue; // pas 2 exemplaires de la même lignée
            usedLines.add(def.evolutionLine);
            picked.push(inst);
          }
          if (picked.length < 9) {
            _showToast('Il te faut au moins 9 lignées différentes dans ta collection.', 'error');
            return;
          }
          renderDefilePlanning(picked.map(inst => inst.instanceId));
          showScreen('defile-planning');
          CWAudioSystem.playCombat();
          return;
        }
        if (mode === 'casting') { showScreen('casting'); return; }
        if (mode === 'fashion-week') { showScreen('fashion-week-roster'); return; }
        showScreen('combat');
  ...
  function showScreen(screenId) {
    ...
    // Défilé en cours (planification, déroulé, résultat OU récompenses) :
    // impossible de sortir tant que la séquence entière n'est pas terminée.
    const DEFILE_FLOW_SCREENS = ['defile-planning', 'defile-playback', 'defile-result', 'defile-rewards'];
    if (_defileInProgress && !DEFILE_FLOW_SCREENS.includes(screenId)) {
      _showToast('Impossible de quitter le Défilé en cours !', 'error');
      return;
    }
...
Le Catalogue affiche aussi les <b>lignées d'évolution</b> — une actrice peut évoluer en progressant en niveau.`,
  ...
  let _defileInProgress = false; // true dès la planification jusqu'à la fin des récompenses — verrouille la navigation
  ...
  async function renderDefileRewards() {
    ...
    if (!el || !_defileRewardsPlan) { _defileInProgress = false; _showCombatSelect(); return; }
    ...
    doneBtn.addEventListener('click', () => {
      _defileInProgress = false; // la séquence est réellement terminée, navigation débloquée
      CWAudioSystem.playGlobal();
      _showCombatSelect();
    });
  }
  ...
  function renderFashionWeekRoster() {
    ...
    // Une run déjà en cours ? On y retourne directement, pas de nouveau tirage.
    if (state.player.fashionWeekRun?.active) {
    ...
    _fwRosterSelection = [];
    _fwRosterSeen = []; // toutes les candidates déjà proposées (choisies ou non), jamais reproposées
    _fwRenderRosterRound(cfg);
  }

  /** Affiche UN tour de sélection (3 candidates, choix de 1) */
  function _fwRenderRosterRound(cfg) {
    const el = document.getElementById('screen-fashion-week-roster');
    const round = _fwRosterSelection.length; // 0, 1, 2
    const candidates = CWGameState.proposeFashionWeekRoundCandidates(_fwRosterSeen);
    ...
    el.innerHTML = `
      ...
      ${_fwRosterSelection.length > 0 ? `
        <div class="fw-roster-picked-row">
          Déjà choisies : ${_fwRosterSelection.map(iid => CWGameState.getCharDef(CWGameState.getPlayerChar(iid).charId).name).join(', ')}
        </div>
      ` : ''}
    `;
  ...
  function renderFashionWeekMap() {
    ...
    if (!run || !run.active) {
      showScreen('fashion-week-roster');
      return;
    }
    ...
    el.innerHTML = `
      ...
      <button class="fw-cashout-btn" id="fw-cashout-btn">🏳️ Arrêter la run et encaisser (${run.totalScore} pts)</button>
    ...
    document.getElementById('fw-cashout-btn')?.addEventListener('click', () => {
      const ok = confirm(`Arrêter la run maintenant et encaisser ${run.totalScore} points de score ? Irréversible.`);
      if (!ok) return;
        ...
        <div class="modal-box fw-dialogue-modal">
          ...
          <div class="fw-dialogue-options">
            ${scenario.options.map((opt, i) => `
              <button class="fw-dialogue-option-btn" data-opt="${i}">
                <span class="fw-dialogue-option-label">${opt.label}</span>
                <span class="fw-dialogue-option-preview">${_fwFormatOutcomePreview(opt.outcome, cfg)}</span>
              </button>`).join('')}
          </div>
        </div>
      </div>`;
  ...
  /** Décrit brièvement ce qu'une issue de Dialogue rapporte, affiché à côté du choix AVANT de cliquer */
  function _fwFormatOutcomePreview(outcome, cfg) {
    if (!outcome) return '';
    const targetSuffix = outcome.target === 'team' ? ' (équipe)' : outcome.target === 'choice_one' ? ' (au choix)' : '';
    switch (outcome.type) {
      case 'levelUp': return `📈 +${outcome.amount} Niv.${targetSuffix}`;
      case 'statBoost': return `✨ +${outcome.amount} ${FW_STAT_LABEL[outcome.stat]}${targetSuffix}`;
      case 'evolve': return `🦋 Évolution${targetSuffix}`;
      case 'currencyGain': return `${cfg.currencyIcon} +${outcome.amount}`;
      case 'runBuff': return `🌟 Bonus de run`;
      case 'formLoss': return `💔 -${outcome.amount} Forme`;
      case 'formGain': return `💗 +${outcome.amount} Forme`;
      case 'triggerDefile': return `⚔️ Défi`;
      case 'gamble': {
        const succ = _fwFormatOutcomePreview(outcome.success, cfg);
        const fail = _fwFormatOutcomePreview(outcome.fail, cfg);
        return `🎲 ${Math.round(outcome.chance*100)}% : ${succ} · sinon ${fail}`;
      }
      default: return '';
    }
  }
  ...
  async function _fwResolveDialogueAndReveal(layerIdx, nodeIdx, optionIdx, chosenMemberId, node, cfg) {
    ...
    if (result?.type === 'triggerDefile' || result?.sub?.type === 'triggerDefile') {
      _fwStartDefileEncounter('adhoc'); // ouvre la planification manuelle — la suite est gérée par ce flux, pas ici
      return;
    }
  ...
  function renderFashionWeekGala() {
    ...
    `;
    const finalAmount = state.player.fashionWeekCurrency || 0;
    _animateCountUp('fw-gala-currency-num', 0, finalAmount, 1400).then(() => {
      document.getElementById('fw-gala-done').disabled = false;
      CWAudioSystem.playSfx(CWAudioSystem.SFX_KEYS.defileVictory);
    });
    document.getElementById('fw-gala-done')?.addEventListener('click', () => _showCombatSelect());
  }
